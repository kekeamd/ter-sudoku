...
\begin{itemize}
    ...
    \item \texttt{pickle} : permet de sauvegarder directement des objets Python dans un fichier \cite{python},
    \item \texttt{matplotlib} : pour la visualisation des statistiques du solveur,
    \item \texttt{selenium} : afin de récupérer une estimation de la difficulté d'une grille depuis le site Sudoku Coach~\cite{sudokucoach},
    \item \texttt{webdriver\_manager} : avec comme utilité d'adapter la récupération de difficulté aux différents navigateur web,
    \item \texttt{flask} : pour pouvoir interagir avec une interface plus pratique et intuitive \cite{flask},
...
\subsubsection{Solve/backtrack/generation humaine}

\subsection{Illustration du fonctionnement}
