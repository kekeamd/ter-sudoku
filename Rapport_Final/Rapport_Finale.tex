...
\texttt{Solver}. Deux familles de solveurs en héritent : \texttt{SolverBacktrack}, qui
implémente une résolution exhaustive par retour sur trace (\textit{backtracking}), et
\texttt{SolverHuman}, qui simule les raisonnements humains classiques à travers une
succession de techniques de complexité croissante. Chaque famille dispose d'une
variante instrumentée (\texttt{SolverBacktrackStats} et \texttt{SolverHumanStats}) permettant
...
\paragraph{Objet \texttt{Grille} : structure d'entrée commune}

Toutes les classes de la hiérarchie reçoivent en entrée un unique paramètre : un objet de
type \texttt{Grille}. Cet objet encapsule l'intégralité de l'état courant de la grille de
...
\paragraph{Classe mère : \texttt{Solver}}

 est une classe abstraite (\texttt{ABC}) qui définit le contrat commun à
toutes les stratégies de résolution. Elle ne possède aucun attribut d'instance et ne stocke
aucun état.

\subparagraph{Méthode abstraite \texttt{solveGrille}}
...
\begin{tabular}{lp{9cm}}
  ...
  \midrule
  \texttt{grille} & Objet \texttt{Grille} en entrée/sortie — modifié en place par le solveur \\
...
\paragraph{ La classe \texttt{SolverBacktrack}}



ne définit aucun attribut propre : toute l'information réside dans
l'objet \texttt{Grille} passé en paramètre. Le parcours s'effectue en coordonnées $(i, j)$
...
sous forme de liste Python, puis \textbf{mélangées aléatoirement} via \texttt{random.shuffle}
...
\subparagraph{Classe SolverBacktrackStats}:
 hérite de \texttt{SolverBacktrack} et ajoute un dictionnaire
de classe partagé entre toutes les instances, avec les clés suivantes :
...
\paragraph{Classe \texttt{SolverHuman}}

\subparagraph{\texttt{SolverHuman} : résolution par techniques humaines}

Contrairement au backtracking, \texttt{SolverHuman} exploite les \textbf{candidats} de chaque
...
\begin{itemize}[noitemsep]
  \item \textbf{Régions} (\texttt{getRegions()}) — liste cyclique (zone / ligne / colonne) ;
  \item \textbf{Candidats} — liste d'entiers par cellule, modifiée en place pendant la résolution ;
  \item \textbf{Valeurs} — entier $\in \{0, 1, \ldots, N\}$ par cellule ($0$ = vide).
\end{itemize}
...
\subparagraph{\texttt{SolverHumanStats} : variante instrumentée}

contrairement à \texttt{SolverBacktrackStats}, les statistiques sont ici des variables
\textbf{locales} initialisées à chaque appel de \texttt{solveWithStats}, ce qui évite les
problèmes de persistance entre résolutions. Chaque entrée de l'historique (\texttt{"history"})
