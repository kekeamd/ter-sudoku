...
\begin{itemize}
    ...
    \item \texttt{selenium}  \textbf{\cite{coursAGL}}: afin de récupérer une estimation de la difficulté d'une grille depuis le site Sudoku Coach \textbf{\cite{sudokucoach}},
...
\subsubsection{Parser   (\textbf{Figure~\ref{fig:parser}})}

Le Parser est une classe faite en 2 parties comme nous pouvons le voir dans le Diagramme. La majorité de la Classe est statique, mais une partie nécessite l’instanciation.

\textbf{1. Le Parser :}

La classe Parser (Fichier Parser.py) a comme objectif de faire du transtypage. Dans notre projet, nous avions comme ambition de pouvoir faire de l’enregistrement de partie, afin de pouvoir sauvegarder une progression par exemple. Mais également l’idée de stocker le déroulement d’une partie ou bien tout le processus de résolution. Également, dans le Scrapper par exemple nous avons besoin d’avoir notre Grille sous forme textuelle, c’est donc ici que le Parser à vue une raison de naître. Il peut donc faire tout un tas de conversion, nous pouvons le voir dans notre Diagramme UML (\textbf{Figure~\ref{fig:parser}}). 
Des exemples sont grilleToTab, tabToGrille, grilleToStringWithoutCandidates, il y a donc, évidemment, des moyens de faire les transtypages dans le deux sens, grilleToTab transforme notre Grille en tableau à deux dimensions, et tabToGrille fait l’inverse. Sachant que les candidats, au début du projet n’étaient pas encore bien mis en place. 
La classe Parser, prend dans la majorité de ses méthodes pas en compte les candidats, les méthodes citées précédemment en font partie (grilleToTab…). Comme dit précédemment, c’est une classe en grande partie statique, en réalité tout ce qui ne touche pas à l’écriture et la lecture de fichier est statique, ce qui est cohérent, car il n’y a besoin de rien pour faire du simple transtypage. 
Cependant, lorsque nous voulons faire de l’écriture/lecture dans un fichier, on a deux méthodes : grilleToFile et fileToGrille ; qui respectivement permette la lecture et l’écriture d’une grille dans un fichier. Ici, l’instanciation sera nécessaire, car nous utilisons une classe FileInteraction qui va être décrite ci-dessous.

\textbf{2. FileInteraction}

La classe FileInteraction a été créée afin de faciliter l’écriture et la lecture dans un même répertoire ou dans un même fichier tout en minimisant le temps d’ouverture des fichiers. On y trouvera des méthodes pour choisir l’emplacement de lecture et d’écriture (setDirectory, setFile…) et des méthodes d’écriture et de lecture (read, write…). Ici, les méthodes de lecture et d’écriture ne revoient pas de manière brute le texte lu, on renvoie un tableau de chaque ligne afin de faciliter le traitement notamment dans Parser.
...
Voici le diagramme de cas d'utilisation UML : \textbf{Figure~\ref{fig:CasUML}}
...
Dans ce cas, chaque entrée contient la grille de jeu, la solution complète, la difficulté et les statistiques associées comme déjà mentionné plus haut dans le rapport.
...
Voici un graphique de performance de backtrack d'une grille Facile : \textbf{Figure~\ref{fig:backtrack-facile}}

Voici un graphique de performance de solveur humaine d'une grille Facile : \textbf{Figure~\ref{fig:solH-facile}}

Voici un graphique de performance de backtrack d'une grille Difficile : \textbf{Figure~\ref{fig:backtrack-difficile}}

Voici un graphique de performance de solveur humaine d'une grille Difficile : \textbf{Figure~\ref{fig:solH-difficile}}

\subsection{Analyse des résultats}
Dans la section au dessus nous avons montré différents graphiques de performances de notre SolverBacktrack et SolverHuman en fonctions des difficultés d’une grille. Premièrement pour le backtracking on a le temps en millisecondes et le nombre d’appels récursifs, on constate que les courbes montrent des temps très faibles sur la majorité des tests (quelques millisecondes) mais des pics énormes sur certains tests (jusqu’à ~1.8 secondes), ce qui montrent que le backtracking est extrêmement instable en terme de performances, il peut résoudre une grille en 0.01 seconde ou en 2 secondes selon la configuration initiale. 
De même pour le nombre d’appels récursifs, les courbes montres des tests avec 500 appels et des pics à ~20000. Ce qui est compréhensible car une seule case mal placée peut multiplier le nombre d’appels par 40.

Pour le SolverHuman comme le bactracking a des temps très faible et contrairement à ce dernier lui est très stable et n’a pas de pics extrêmes. Il applique des techniques déterministes donc il est rapide et constant.

\subsection{Comparaison des méthodes}

\subsubsection{Backtracking vs Méthodes humaines}
Les deux approches présentent des philosophies et des comportements très différents.

Le Backtracking est un solveur universel : il peut résoudre n’importe quelle grille valide, indépendamment de sa complexité logique. Cependant, cette universalité a un coût : l’algorithme peut s’effondrer en performance dès que la grille nécessite une exploration profonde de l’arbre des possibilités. Les résultats montrent des variations extrêmes, avec des temps d’exécution pouvant être multipliés par 100 ou 200 selon les cas. Le Backtracking est donc fiable mais imprévisible.

À l’inverse, le SolverHuman est extrêmement rapide et stable. Il ne souffre d’aucune explosion combinatoire et son temps d’exécution reste quasi constant. En revanche, il est limité par les techniques implémentées : si une grille nécessite une technique non disponible, le solveur restera bloqué. Il n’a pas la garantie de résoudre toutes les grilles, mais il excelle dans l’analyse logique et la classification de difficulté.

En résumé, les deux solveurs sont complémentaires :

- le Backtracking est idéal pour vérifier l’unicité d’une solution ou générer des grilles complètes,

- le SolverHuman est parfait pour analyser la difficulté, reproduire une résolution humaine et garantir des performances stables.
...
Cependant cela résulta en un projet certes fonctionnel bien que rudimentaire mais qui laissait à désirer, qui nous laissa insatisfait après cette fin de sprint. Nous avons donc décidé de réorganiser le projet, de mettre des règles, conventions et répartitions de tâches ainsi que d'établir un diagramme des classes, sans oublier des réunions hebdomadaires avec notre encadrant afin faire le point et de rester dans la bonne lignée.
...
Les fonctionnalités qui ont été implémenter sont les suivantes :
    
- Génération de grille via Backtrack,

- Résolution d'une Grille Sudoku via Backtrack,

- Génération de grille via Techniques Humaines,

- Résolution d'une Grille Sudoku via Techniques Humaines,

- Estimation de difficulté d'une Grille via Techniques Humaines,

- Le traçage de la résolution d'une Grille,

- Un historique permettant d'enregister et de restituer une séquence de Grille,

- Un Parser permettant le transtypage notamment pour la Grille,

- Un système de sauvegarde et de restitution de Grille précédemment généré,

- Un système permettant de récupérer l'estimation de la difficulté faite par un site externe : SudokuCoach \textbf{\cite{sudokucoach}},

- Un affichage console permettant l'affichage et l'interaction avec les fonctionnalités implémentés ainsi que jouer,

- Demande de génération d'affichage de Grille dans une page web,
   
- Un système de modification visuelle de Grille sans en modifier la difficulté.

\subsection{Comparaison avec cahier des charges}
Le cahier des charges qui fut décidé incluais :
    
- Génération de Grille valide via backtrack en ayant une difficulté dépendante du nombre de cases vides,

- Un Parser de fichier,

- Un solver automatique (backtrack),

- Une interface minimal de test,

- Une estimation de difficulté via le nombre de cases vides.

Nous avons pris une direction plus orientée Résolution, estimation et génération via des techniques Humaine afin d'obtenir une difficulté plus Cohérente et également indiqué une manière de résoudre à échelle humaine.

Nous avons réalisé une interface Console plus claire et avec plus de fonctionnalités que nous l'avions décidé dans le cahier des charges. Également le Parser contient plus de fonctionnalités que prévu tout en remplissant évidemment sa fonction demandée. Le "Scrapping" afin d'obtenir une estimation de difficulté via des sources externe n'était pas dans le cahier des charges initial, de la même manière la modification de Grille, la sauvegarde de Grille, l'historique, n'y étaient pas non plus. 
Nous avons donc respecté le cahier des charges tout en améliorant des fonctionnalités et en ajoutant de nouvelles.

\subsection{Perspectives et points ouvertes}
Cependant, le projet pourrait être cible d'encore multiples améliorations, ajouts et optimisations, tels que l'ajout et l'optimisation de techniques de résolution, l'optimisation de la génération de grilles et même l'amélioration de l'interface.

\subsubsection{Amélioration des algorithmes}
En effet, nous ne faisons qu'écorcher la surface de ce qu'est l'iceberg des techniques de résolution, un bon exemple de cela est le site Sudoku Coach \textbf{\cite{sudokucoach}} qui regorge de ces techniques, de la plus simple comme le singleton nu à la plus complexe tel que méduse 3D. 

De plus, les techniques actuellement implémentées ne sont pas forcément optimales et certaines mesures à prendre pour les optimiser serait sûrement possible, tel que la méthode de Grille renvoyant les régions que nous avons ajouté.

\subsubsection{Simplification de la génération de grille}
Comme énoncé précédemment, la génération de grille est grandement optimisable, nous avons discutés d'une tactique astucieuse à ce sujet: la transformation de grille. Cela consiste à avoir une banque de grilles triées par difficulté, puis, lorsque nous souhaitons générer une grille, de saisir une grille de cette banque de difficulté équivalente et de la passer à travers une fonction de transformation qui peut aléatoirement faire:
	
- une rotation d'un certain angle (ou symétrie centrale),

- une symétrie axiale,

- et une permutation des valeurs.

Cela permettrai de réduire considérablement le temps de génération des grilles.

\subsubsection{Interface Web}
Les plus grands défauts de l'interface console sont ses interactions limitées et non intuitives ainsi que l'affichage restreint, une bonne solution à ces problèmes serait une interface web, qui marie simplicité et visuel. 

Cependant il nous faudrait pour cela un fichier main alternatif sur lequel tourne un serveur afin de pouvoir communiquer entre les modules du sudoku et l'interface web.

\subsubsection{Parser}
En prenant du recul, cette classe aurait pu être encodée totalement différemment, notamment car il existe déjà des bibliothèques qui facilitent l'écriture et la lecture de données encodées dans des fichiers, mais également qui facilitent le transtypage (ici je pense à la bibliothèque JSON). Ce qui remets également en cause la question de l'existence de la classe FileInteraction, classe qui aurait pu être faite différemment ou alors simplement supprimée.
...
\bibitem{coursAGL}
\textbf{Cours d'Ateliers de Génie Logiciel}. 
Cours 4 : End2End Testing et Selenium, L3 Informatique, Université de Montpellier. 
Utilisé pour le sudoku scraping.\\
Support de cours fourni au format PDF : \texttt{CoursE2ETesting\_Selenium.pdf}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Diagramme_classe/DiagrammeSlice/Interface.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.10\textwidth]{DiagrammeDeCasD_utilisation.png}
    \caption{Le diagramme de cas d'utilisation UML}
    \label{fig:CasUML}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Diagramme_classe/Diagramme_simpl.png}
    \caption{Le diagramme de classes UML entier et simplifié}
    \label{fig:UML}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{graphiques/backtrackFacile(ms).png}
    \caption{Graphique de performance de backtrack sur une grille Facile}
    \label{fig:backtrack-facile}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{graphiques/SolverHumanFacile(ms).png}
    \caption{Graphique de performance de solveur humaine sur une grille Facile}
    \label{fig:solH-facile}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{graphiques/BacktrackDifficile(ms).png}
    \caption{Graphique de performance de backtrack sur une grille Difficile}
    \label{fig:backtrack-difficile}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\textwidth]{graphiques/SolverHumanDifficle(ms).png}
    \caption{Graphique de performance de solveur humaine sur une grille Difficile}
    \label{fig:solH-difficile}
\end{figure}
