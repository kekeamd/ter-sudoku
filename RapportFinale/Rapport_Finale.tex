...
InterfaceConsole.startPlaying() → playSudoku() → GrilleHuman.generateValuesHumanRated() → generateValuesDifficile() → création d’un clone test → SolverHumanStats.solveWithStats(test, ...) → appels successifs aux techniques de SolverHuman (dernierNombre, singletonNu, singletonCache, paireNu, paireCachee, candidatEnferme). 
